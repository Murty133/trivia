...
def create_app(test_config=None):
  ...
  def submit_search():
    ...
    try:
      ...
      formatted_questions = [question.format() for question in questions]
      questions_by_categories = Question.query.filter(
        Question.question.ilike(f'%{search_term}%')).distinct(Question.category)
      current_category_ids = [
        question.category for question in questions_by_categories]
      current_category = []
      for id in current_category_ids:
        c = Category.query.filter(Category.id == id).first()
        current_category.append(c.type)
